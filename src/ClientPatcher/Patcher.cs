...
/// <summary>
/// Orchestration runner (design §8, Data Flow). Holds the full parse-free
/// pipeline — validate → resolve → read → engine → backup → write → report —
/// behind a single testable entry point that writes to caller-supplied streams
/// and returns the process exit code. <see cref="Program"/> is a thin shell that
/// parses args and delegates here so the exit-code mapping is identical whether
/// invoked from <c>Main</c> or a test.
/// </summary>
public static class Patcher
{
    ...
    /// <summary>
    /// Run the patch pipeline for already-parsed <paramref name="options"/>, writing
    /// the report to <paramref name="stdout"/> and any errors/warnings to
    /// <paramref name="stderr"/>. Returns the process exit code (design §8):
    /// 0 ok · 2 validation · 3 not-found · 4 IO. Assumes <c>--help</c> has already
    /// been handled by the caller.
    /// </summary>
    public static int Run(PatchOptions options, TextWriter stdout, TextWriter stderr)
    {
        void WriteError(string message) => stderr.WriteLine("error: " + message);

        // 1. Validate; errors → exit 2 (nothing written). Warnings are printed but
        ...
        // 2. Resolve target files under --client. None → exit 2.
        ...
        return RunPatch(options, targets, stdout, WriteError);
    }
        ...
        Action<string> writeError)
    {
        ...
        ReportWriter.Write(stdout, results, endpoint);
